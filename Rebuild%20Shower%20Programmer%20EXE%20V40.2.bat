...
rem - Verifies the centralized V40 version, changelog, GUI, updater, and package contracts before building.
...
rem - BUILD FIX V40.1: reads version.json through a temporary file so quoted Python paths work in CMD.
rem - BUILD FIX V40.2: fixes the packaged self-test validation print expression.
rem ================================================================
...
if not exist "%SOURCE_CHANGELOG%" (
    echo ERROR: Missing required file:
    echo   %SOURCE_CHANGELOG%
    goto failed
)
...
echo Validating release version and changelog...
rem Do not run a quoted Python path inside FOR /F command substitution. CMD can
rem strip the opening quote and turn the executable plus -c into one invalid
rem command. Write the three values once, then read the temporary text file.
set "VERSION_INFO_FILE=%TEMP%\shower_programmer_version_%RANDOM%_%RANDOM%.txt"
if exist "%VERSION_INFO_FILE%" del /F /Q "%VERSION_INFO_FILE%" >NUL 2>NUL
%PY_CMD% -c "import json, pathlib; d=json.loads(pathlib.Path(r'%SOURCE_VERSION%').read_text(encoding='utf-8')); print('APP_VERSION=' + str(d['version']).strip()); print('APP_VERSION_MARKER=' + str(d['marker']).strip()); print('APP_RELEASE_NAME=' + str(d['release_name']).strip())" > "%VERSION_INFO_FILE%"
if errorlevel 1 (
    echo ERROR: Python could not read %SOURCE_VERSION%.
    if exist "%VERSION_INFO_FILE%" del /F /Q "%VERSION_INFO_FILE%" >NUL 2>NUL
    goto failed
)
for /f "usebackq tokens=1,* delims==" %%A in ("%VERSION_INFO_FILE%") do (
    if /I "%%A"=="APP_VERSION" set "APP_VERSION=%%B"
    if /I "%%A"=="APP_VERSION_MARKER" set "APP_VERSION_MARKER=%%B"
    if /I "%%A"=="APP_RELEASE_NAME" set "APP_RELEASE_NAME=%%B"
)
if exist "%VERSION_INFO_FILE%" del /F /Q "%VERSION_INFO_FILE%" >NUL 2>NUL
if not defined APP_VERSION (
    echo ERROR: Could not read version from %SOURCE_VERSION%.
    goto failed
)
if not defined APP_VERSION_MARKER (
    echo ERROR: Could not read marker from %SOURCE_VERSION%.
    goto failed
)
%PY_CMD% -c "import json, pathlib; v=json.loads(pathlib.Path(r'%SOURCE_VERSION%').read_text(encoding='utf-8')); required=('version','version_number','marker','release_name','release_date'); missing=[k for k in required if not str(v.get(k,'')).strip()]; assert not missing, missing; c=pathlib.Path(r'%SOURCE_CHANGELOG%').read_text(encoding='utf-8'); assert '## ['+str(v['version'])+']' in c, 'Missing changelog heading'; print('  Release ' + str(v['version']) + ': ' + str(v['release_name']))"
if errorlevel 1 goto failed
...
findstr /C:"%APP_VERSION_MARKER%" "%SOURCE_GUI%" >NUL
if errorlevel 1 (
    echo ERROR: Backend\shower_programmer_gui.py does not contain the current release marker:
    echo   %APP_VERSION_MARKER%
    goto failed
)
...
if not exist "%STAGED_DIR%\_internal\_tcl_data" if not exist "%STAGED_DIR%\_internal\tcl_data" (
    echo ERROR: The staged build is missing Tcl runtime data required by Tkinter.
    goto failed
)
if not exist "%STAGED_DIR%\_internal\_tk_data" if not exist "%STAGED_DIR%\_internal\tk_data" (
    echo ERROR: The staged build is missing Tk runtime data required by Tkinter.
    goto failed
)
...
%PY_CMD% -c "import json, pathlib; p=pathlib.Path(r'%SELF_TEST_REPORT%'); d=json.loads(p.read_text(encoding='utf-8')); assert d.get('ok') is True, d; assert d.get('version') == r'%APP_VERSION_MARKER%', d; assert d.get('display_version') == r'%APP_VERSION%', d; print('  Packaged EXE self-test passed for ' + r'%APP_VERSION%' + '.')"
if errorlevel 1 (
...
if defined BUILD_SHA (
    >"%STAGED_DIR%\.shower_update.json" echo {"sha":"%BUILD_SHA%","version":"%APP_VERSION%","release_name":"%APP_RELEASE_NAME%","exe_sha256":"%EXE_SHA256%","gui_sha256":"%GUI_SHA256%","gui_version":"%APP_VERSION_MARKER%","built_at":"%BUILD_TIME%","method":"build"}
) else if defined EXE_SHA256 (
    >"%STAGED_DIR%\.shower_update.json" echo {"version":"%APP_VERSION%","release_name":"%APP_RELEASE_NAME%","exe_sha256":"%EXE_SHA256%","gui_sha256":"%GUI_SHA256%","gui_version":"%APP_VERSION_MARKER%","built_at":"%BUILD_TIME%","method":"build"}
)
...
if not exist "%FINAL_DIR%\_internal\_tcl_data" if not exist "%FINAL_DIR%\_internal\tcl_data" (
    echo ERROR: Missing Tcl runtime data after deployment.
    goto failed
)
if not exist "%FINAL_DIR%\_internal\_tk_data" if not exist "%FINAL_DIR%\_internal\tk_data" (
    echo ERROR: Missing Tk runtime data after deployment.
    goto failed
)
...
if not exist "%FINAL_DIR%\_internal\Backend\version.json" (
    echo ERROR: Missing required file after deployment:
    echo   %FINAL_DIR%\_internal\Backend\version.json
    echo The application version record was not included in the bundled runtime.
    goto failed
)
...
echo ========================================
echo.
echo Version: %APP_VERSION% - %APP_RELEASE_NAME%
echo.
echo EXE:
echo   %CD%\%FINAL_EXE%
echo.
